...
namespace
{
std::pair<std::unique_ptr<uint8_t[]>, CodeAnalysis::JumpdestMap> analyze_jumpdests(
    const uint8_t* code, size_t code_size, evmc_opcode final_opcode)
{
    ...
    // Used to terminate invalid jumps, see jump().
    padded_code[code_size] = static_cast<uint8_t>(final_opcode);
    // Set final STOP/INVALID at the code end - guarantees loop termination.
...
{
    auto code_and_jumpdest_map = analyze_jumpdests(code, code_size, OP_STOP);
